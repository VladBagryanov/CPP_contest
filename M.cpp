...
class Tree {
 ...
 public:
  ...
  Node* MaxSearch(Node* temp);
};

Node* Tree::MaxSearch(Node* temp) {
  if (temp->right == nullptr) {
    return temp;
  }
  return MaxSearch(temp->right);
}
...
int main() {
  ...
  Node* max = tree.MaxSearch(tree.Root());
  if (max->left == nullptr) {
    std::cout << tree.MaxSearch(max)->parent->value;
  } else {
    std::cout << tree.MaxSearch(max->left)->value;
  }
